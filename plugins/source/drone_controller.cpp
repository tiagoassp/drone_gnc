...
namespace gazebo
{
  ...
  // Load the controller
  void DroneController::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf)
  {
    ...
    robot_namespace_ = "/drone";

    ROS_INFO_NAMED("3d_move", "Starting Controller");

    if (!sdf->HasElement("robotNamespace"))
    {
      ROS_INFO_NAMED("3d_move", "PlanarMovePlugin missing <robotNamespace>, "
    ...
    {
      ROS_WARN_NAMED("3d_move", "PlanarMovePlugin (ns = %s) missing <commandTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("3d_move", "PlanarMovePlugin (ns = %s) missing <odometryTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("3d_move", "PlanarMovePlugin (ns = %s) missing <odometryFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("3d_move", "PlanarMovePlugin (ns = %s) missing <robotBaseFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN_NAMED("3d_move", "PlanarMovePlugin (ns = %s) missing <odometryRate>, "
          "defaults to %f",
    ...
    {
      ROS_WARN_NAMED("3d_move", "PlanarMovePlugin (ns = %s) missing <cmdTimeout>, "
          "defaults to %f",
    ...
    {
      ROS_FATAL_STREAM_NAMED("3d_move", "PlanarMovePlugin (ns = " << robot_namespace_
    ...
    ROS_DEBUG_NAMED("3d_move", "OCPlugin (%s) has started",
    ...
    ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
        boost::bind(&DroneController::cmdVelCallback, this, boost::placeholders::_1),
        ros::VoidPtr(), &queue_);

    vel_sub_ = rosnode_->subscribe(so);
    odometry_pub_ = rosnode_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

    // start custom queue for 3d drive
    callback_queue_thread_ =
      boost::thread(boost::bind(&DroneController::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ =
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&DroneController::UpdateChild, this));

  }

  // Update the controller
  void DroneController::UpdateChild()
  {
    #ifdef ENABLE_PROFILER
        IGN_PROFILE("DroneController::UpdateChild");
    ...
    // Get full orientation (yaw, pitch, and roll)
    float yaw = pose.Rot().Yaw();
    float pitch = 0;//pose.Rot().Pitch();
    float roll = 0;//pose.Rot().Roll();

    // Convert desired velocities from the drone's local frame to the world frame
    // Assuming x_, y_, and z_ are the desired velocities in the local frame of the drone
    float vx = x_ * (cos(yaw) * cos(pitch)) + y_ * (cos(yaw) * sin(pitch) * sin(roll) - sin(yaw) * cos(roll)) + z_ * (cos(yaw) * sin(pitch) * cos(roll) + sin(yaw) * sin(roll));
    float vy = x_ * (sin(yaw) * cos(pitch)) + y_ * (sin(yaw) * sin(pitch) * sin(roll) + cos(yaw) * cos(roll)) + z_ * (sin(yaw) * sin(pitch) * cos(roll) - cos(yaw) * sin(roll));
    float vz = x_ * (-sin(pitch)) + y_ * (cos(pitch) * sin(roll)) + z_ * (cos(pitch) * cos(roll));

    // Set the transformed velocities
    parent_->SetLinearVel(ignition::math::Vector3d(vx, vy, vz));

    // Set the desired angular velocity
    // rot_ should be a 3D vector (roll, pitch, yaw) for 3D angular velocity
    parent_->SetAngularVel(ignition::math::Vector3d(roll_, pitch_, rot_));
  ...
  void DroneController::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void DroneController::cmdVelCallback(
  ...
  void DroneController::QueueThread()
  {
  ...
  void DroneController::publishOdometry(double step_time)
  {
